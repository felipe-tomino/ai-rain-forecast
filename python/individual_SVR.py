# este algoritmo cria um regressor SVR por sensor
import psycopg2
...
for gauge_id in gauges_ids:
    ...
    regr[gauge_id] = SVR(kernel='rbf', C=100, gamma=0.001)
    regr[gauge_id].fit(X_train[gauge_id], y_train[gauge_id])
    pickle.dump(regr[gauge_id], open('./SVR_regressors/' + str(gauge_id), 'wb'))
...
with open('./results/individual_SVR.csv', 'w') as f:
